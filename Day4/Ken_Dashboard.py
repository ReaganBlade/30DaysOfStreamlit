...
def load_data():
    ...
    # Fixing the date parsing issue
    df_agg['Video publish time'] = pd.to_datetime(df_agg['Video publish time'], format='%b %d, %Y', errors='coerce')
    
    # Convert the Average view duration to total seconds
    df_agg['Average view duration'] = pd.to_timedelta(df_agg['Average view duration']).dt.total_seconds()
    
    df_agg['Avg_duration_sec'] = df_agg['Average view duration']
